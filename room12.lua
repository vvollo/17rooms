...
room {
	...
	n_to = 'room2_on_terrasa';
	out_to = 'room2_on_terrasa';
	e_to = 'room13_library';
	s_to = 'room10_zal';
	w_to = 'room11_kabinet';
	...
	before_Wait = "Ты немного подождала. Внезапно из шкафа послышался какой-то неясный звук. Мыши наверное.";
	before_Taste = 'Как-то в детстве ты лизнула железные качели в декабре. С тех пор стараешься явно несъедобные объекты не лизать.';
	...
	compass_look = function(s,dir)
		if _'room12_портал':has'on' then
			if dir == 'n_to' or dir == 'out_to' then
				p 'В двери, ведущей на север, видны бесконечные морские просторы без каких-либо признаков жизни. Конечно, проверять голограмма ли это или правда, а если даже и второе, то есть ли вход с той стороны, ты не будешь. Тем более, плаваешь ты посредственно.';
			elseif dir == 'w_to' then
				p 'В двери, ведущей на запад, видна какая-то пустыня, только какая-то серая и со странным резким освещением. Ты провела взглядом по странно близкому горизонту и зависла над невозможной картиной. Из-за горизонта неспешно выкатывался сине-зеленый шар планеты с очень узнаваемыми очертаниями материков. Ты в испуге отшатнулась.';
			elseif dir == 'e_to' and _'room12_ноутбук'.broken then
				p 'В двери, ведущей на восток, видна заметенная снегом поляна и чугунный фонарь. Внезапно из темноты за кругом света от фонаря выступило жуткое создание с большими круглыми глазами. Неестественное и отвратительное. И оно движется к тебе!';
	...
	['before_Walk,Enter'] = function(s,w)
		if _'room12_портал':has'on' then
			if mp:compass_dir(w) == 'n_to' or mp:compass_dir(w) == 'w_to' or mp:compass_dir(w) == 'out_to' then
	...
	before_ThrowAt = function(s,w,dir)
		if w == _'room2_black_rock' and (dir^'@s_to' or dir^'room12_sun') then
			...
			mp.score=mp.score+5;
			walk 'ending_trueend';
		elseif w == _'room2_black_rock' and (dir^'@n_to' or dir^'@w_to' or dir^'@e_to' or dir^'@out_to' or dir^'room12_decor' or dir^'room12_stranga') then
	...
	before_PushDir = function(s,w,dir)
		...
		elseif w == _'room16_witch' and (dir^'@n_to' or dir^'@w_to' or dir^'@e_to' or dir^'@out_to') then
...
obj {
	...
	description = function(s)
		local _txt = "";
		if _'room2_album_8':has'seen_on_high' then
			_txt = "^Внезапно ты вспоминаешь, что один из участников экспедиции в Такла-Макан очень похож на мужчину с фотографии. Должно быть, это его сын или внук. И он, видимо, знаком с тётушкой.";
		end
		return 'На стене висит фотография улыбающегося молодого мужчины, замотанного в бурнус (или как там эта арабская одежда называется?), на фоне морского берега. В руках он держит красный керамический диск, покрытый резьбой в виде круговых орнаментов и неизвестных символов.' .. _txt;
	end;
...
obj {
	...
	before_Take = 'Ты не дотягиваешься. Слишком высоко. Да и зачем?';
}:attr 'scenery';
...
obj {
	...
	['before_Walk,Enter,Open'] = function(s)
		if _'room12_портал':has'on' then
			if s:once() then
				p 'Открыв шкаф, ты увидела вовсе не пустые вешалки, как ожидалось. Вместо задней стенки ты обнаружила ночь, заметенную снегом поляну и чугунный фонарь. Внезапно в темноте за кругом света от фонаря тебе почудилось какое-то движение, из темноты выступили очертания чего-то. Неестественные и отвратительные. Первобытный ужас охватил тебя, холод прокатился по позвоночнику, и ты поспешно захлопнула дверцу шкафа.';
			else
				p 'Нет уж. Там страшно.';
			end;
		else
			p 'Шкаф совершенно пуст. Ты прикрыла дверцы обратно.';
		end;
	end;
	before_Take = 'Ты не думаешь, что сможешь даже на миллиметр подвинуть этот шкаф.';
}:attr 'static';
...
obj {
	...
	obj = {'room12_ноутбук'};
	before_Take = 'Слишком большой, чтобы ты могла справиться с ним.';
...
obj {
	...
	description = function(s)
		if s:has'open' then
			p "На экране светится что-то типа нортон-коммандера. Но почти все директории оказались заблокированы. В документах, в папке 'Пират', ты нашла единственный файл, подписанный 'код'. И ещё загадочное приложение 'портал'.";
	...
	with_key = 'squarekey';
	before_Take = 'Ты попыталась снять кейс со стола, но внутри что-то очень тяжёлое. Даже тяжелее той сумки, что ты собрала в тот раз в отпуск.';
	before_LetIn = 'Положить ключ в кейс и захлопнуть его. Прекрасная идея. Ты как-то с машиной это проделала, пришлось специального слесаря вызывать. Так что как-нибудь потом.';
	before_SwitchOn = 'Каких-то специальных кнопок для включения ноутбука ты не нашла. Может быть, достаточно открыть его?';
	before_SwitchOff = 'Каких-то специальных кнопок для выключения ноутбука ты не нашла. Может быть, достаточно закрыть его?';
...
obj {
	...
	before_Take = 'В ноутбуке есть какие-то разъёмы сбоку, но это явно не USB. Да и флешки у тебя нет с собой.';
}:attr 'scenery';
...
obj {
	...
	before_Default = function(s, ev, w)
		...
		p "Плющ снаружи. И лучше бы ему там и оставаться.";
	end;
}: attr 'scenery':disable()
...
obj {
	...
	before_ThrownAt = function(s, w)
		if(w == _'dagger') then
			return "Метким броском кинжала ты убиваешь уродливую тварь наповал. Прекрасно! Дальше что?"
		else
			return false
		end;
	end;
	before_Shoot = function(s, w)
		...
		if(w == _'gun') then
			_'gun'.loaded = false;
			return "Метким выстрелом ты убиваешь уродливую тварь наповал. Прекрасно! Дальше что?"
		else
			return false
		end;
	end;
