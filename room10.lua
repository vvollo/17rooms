...
room {
	...
	before_Any = function(s, ev, w)
		...
		s.door_ask = false
		if ev == "Yes" then
			p [[Ты толкаешь дверь и она открывается! Что за маньяк проектировал особняк?]]
	...
	obj = {
		...
		obj {
			...
			obj {
				-"табличка|дверная табличка";
				description = [[Дверная табличка. На одной стороне написано "На себя", на второй, соответственно, - "От себя". Интересно, от чего она?]];
				before_Take = [[Тебе не нужна табличка. Достаточно просто знать, что она существует.]];
			}:attr'static';
		};
		door {
		-"лестница,ступен*";
