...
obj {
	...
	after_MakeUse = "Ты не хочешь использовать {#first/вн} для столь прозаических целей.";
}
obj {
...
obj {
	...
	continue = function()
		...
		if(_'room16_c1'.state == 2 and _'room16_c2'.state == 2 and _'room16_c3'.state == 2) then _'room16_AI'.win = 2; end;
		if(_'room16_AI'.win == 2) then
			p ('Ты победно соединяешь три нолика одной кривой линией (так уж получается, когда царапаешь кинжалами шкафы), и вдруг какая-то таинственная сила отталкивает тебя от шкафа так, что часть твоих вещей остаётся внутри. Дверцы шкафа захлопываются.')
			_'room16_AI'.game = 2;
	...
	daemon = function(s)
		...
		if(_'room16_AI'.daemon_stage == 17) then
			p ('Кто-то мог бы сказать, что ты нашла концовку №4/7. Но ты, конечно, можешь с этим не согласиться и играть дальше.')
		end;
...
obj {
	...
	["before_Attack,Cut"] = function(s,w)
		if w and w ~= _'dagger' then
			return false
		end;
		if(pl:have(_'dagger')) then
			if(s.state == 3) then
