...
obj {
	-"картина";
	nam = "room14_picture";
	description = "Портрет молодой тёти Агаты и её мужа. Обнявшись, они стоят в комнате напротив зеркала. Тетя Агата в тёмно-синем вечернем платье, а её муж в строгом фраке. Кажется, они выглядят счастливыми. Наверное, портрет был сделан вскоре после их свадьбы.";
	seen = false;
	before_Exam = function(s)
...
obj {
	...
	before_Search = function(s)
		if not _"room14_dress".worn then
			p "Ты рассматриваешь в зеркале себя и свою непримечательную одежду. Это зеркало видало зрелища и получше.";
		else
			p "Ты рассматриваешь в зеркале себя в вечернем платье. Наверное, и тётя любит так же примерять это платье перед зеркалом.";
		end;
	end;
...
obj {
	...
	before_Push = function(s)
		if not _"room14_dress".worn and not _"room14_door1".seen then
			p"Ты встала перед зеркалом и нажала на кнопку. Раздался тихий щелчок, но ничего не произошло.";
			if not s:once() then
				p " Может, зеркало не слушается тебя, так как ты не похожа на его хозяйку.";
			end;
	...
	['before_Take,Push,Pull,Turn,LookUnder'] = function(s)
		if _"room14_drawer".moving == true then
			...
			if _"room14_carpet".moving == false then
				p"Попробовав поднять ковёр, ты поняла, что он слишком тяжёлый, чтобы переместить его, но, откинув край ковра, ты увидела на полу посреди коридора квадратную пластину. А рядом с пластиной — грязную перчатку горничной. Пол заметно потёрт.";
		...
		if _"room14_box1".stable or _"room14_box2".stable then
			...
			elseif _"room14_carpet".moving == true then
				if not s.moving and _"room14_plate":empty() then
					...
					if not _"room14_platform".seen then
						p " Стрелка весов заметно отклонилась вправо.";
					end
					_"room14_drawer".moving = true;
				...
				elseif s.moving then
					...
					if not _"room14_platform".seen then
						p " Стрелка весов упала обратно до нуля.";
					end
					_"room14_drawer".moving = false;
...
obj {
	...
	description = function(s)
		...
		if not _"room14_carpet".moving then
			p " Накрыт ковром.";
		else
			p " От комода до квадратной пластины протёрт след, как будто что-то тащили.";
		end;
	return false
	end;
	scope = {};
...
obj {
	...
	nam = "room14_showcase";
	description = function(s)
		if _'gun':has'scenery' then
			p[[Стеклянная витрина, за которой находится пистолет с позолоченной рукояткой.]];
			if _"room14_report".seen then
				p[[Возможно, это и есть беретта, упоминавшаяся в отчёте?]];
			end
		else
			p[[Пустая стеклянная витрина.]];
		end
	end;

	before_Exam = function(s)
		_"room14_secondfloor".obj:add("gun");
		return false
	end;
	before_Open = function(s)
		p "Если тебе что-нибудь понадобится в витрине, ты её откроешь.";
	end;
	before_Receive = function(s,w)
...
obj {
	...
	loaded = true;
	description = function(s)
		p"Пистолет с позолоченной рукояткой.";
		if s:has'scenery' then
			if _"room14_report".seen then
				p"Возможно, это и есть беретта, упоминавшаяся в отчёте?";
			else
				p"Ты никогда особо не любила оружие и не разбиралась в нём.";
			end
		end
		if pl:have(s) then
			if s.loaded then
	...
	["before_Take,Remove"] = function(s)
		...
		if s.magneted then
			p "Пистолет невозможно вытащить из витрины, как будто его там что-то держит.";
		else
			p "Подумав, ты решаешь всё же не брать пистолет. Ещё поранишься!";
		end
	end;
	before_Turn = function(s)
		if not _"room14_report".seen then
			p "Ты не понимаешь, зачем это делать.";
		elseif s.magneted then
			p "Повернув пистолет, ты слышишь щелчок. Похоже, теперь его можно взять.";
			s.magneted = false;
		else
			p "Больше не нужно.";
		end
...
obj {
	-"пластина, напольная пластина|весы/жр,мн";
	nam = "room14_plate";
	description = function(s)
		p"Напольная деревянная квадратная пластина. На ней изображена какая-то девушка с находящимися в равновесии весами, на которых стоят пустые бокалы.";
		mp:content(s);
		if not s:empty() then
			p"Сейчас пластина нажата.";
		end
		p " Возле пластины -- шкала со стрелкой и надписью kg. Весы?";
	end;
	before_Any = function(s)
		if _"room14_carpet".moving == false then
	...
	['before_Enter,Walk'] = function(s)
		if _"room14_platform".seen then
			p"Ты встала на пластину. Ничего не произошло. ^Ты сошла с пластины.";
		elseif not _"room14_drawer".moving then
			p"Ты встала на пластину. Пластина нажалась, стрелка отклонилась вправо, но недостаточно, и ничего больше не произошло. Возможно, здесь нужен другой вес. ^Ты сошла с пластины.";
		else
	...
	before_Push = function(s)
		...
		local txt2 = "";
		if not _"room14_platform".seen then
			txt1 = "стрелка слегка отклонилась вправо, ";
			txt2 = " Судя по всему, пластина реагирует на вес предметов.";
		end
		p ("Ты нажала на квадратную пластину, " .. txt1 .. "но ничего не произошло." .. txt2);
	end;
	after_Receive = function(s, w)
		if not _"room14_platform".seen then
			p ("Положив " .. w:noun'вн' .. " на " .. s:noun'вн' .. ", ты замечаешь, что стрелка слегка отклонилась вправо.");
		else
			return false;
		end
	end;
	after_LetGo = function(s, w)
		if not _"room14_platform".seen then
			p ("Взяв " .. w:noun'вн' .. " с " .. s:noun'рд' .. ", ты замечаешь, что стрелка упала обратно до нуля.");
		else
			return false;
		end
	end;
	each_turn = function(s)
		if _'room14_drawer':inside'room14_plate' and _"room14_box1".stable == true and _"room14_box2".stable == true and _"room14_box1":empty() and _"room14_box2":empty() and 
			...
			p"Стрелка остановилась точно на метке. Внезапно раздался щелчок, и пластина нажалась. Похоже, вес предметов оказался подходящим. Из стены рядом с деревянной дверью выдвинулась небольшая платформа.";
		end
	end;
	capacity = 1;
...
obj {
	-"шкала,стрелка,метка,красная метка";
	nam = "room14_scale";
	description = function(s)
		p "Шкала со стрелкой и надписью kg. Слева ноль, посередине -- красная метка.";
		if _"room14_platform".seen then
			p " Стрелка зафиксирована точно на метке.";
		elseif _'room14_plate':empty() then
			p " Сейчас стрелка на нуле.";
		elseif _'room14_drawer':inside'room14_plate' then
			if _"room14_box1".stable and _"room14_box2".stable then
				if _"room14_box1":empty() and _"room14_box2":empty() then
					p "{$fmt em|Найден баг!}";
				else
					p " Сейчас стрелка чуточку правее метки.";
				end
			elseif _"room14_box1".stable or _"room14_box2".stable then
				p " Стрелка достаточно далеко отклонилась вправо, но всё равно слегка не достигает метки.";
			else
				p " Сейчас стрелка примерно на полпути между нулём и меткой.";
			end
		else
			p " Сейчас стрелка слегка отклонилась вправо от нуля, но до метки ей далеко.";
		end
	end;
	before_Any = function(s)
		if not _"room14_carpet".moving then
			return p "Но шкала сейчас под ковром.";
		else 
			return false
		end
	end;
...
obj {
	...
	before_Receive  = function(s, w)
		if w ~= _"piramidekey" then
			p("Ты положила ", w:noun'вн', " на платформу. Но ", w:noun'им', " совершенно не подходит для треугольного углубления, поэтому ты  забрала ", w:it'вн', " обратно.");
		else 
			return false
		end
	end;
...
obj {
	...
	elseif found == 32 then
		p"Ты нашла свадебное фото: вместо традиционного белого тётя была на свадьбе в тёмно-синем вечернем платье.";
	...
	elseif found == 10 then
		p'Ты нашла в документах следующее: "...никаких следов выстрела на руках горничной не осталось, поскольку она была в длинных атласных перчатках... на ладони горничной -- надпись шариковой ручкой: {$fmt b|КФ937}..."';
		if not s.score then
			mp.score=mp.score+1;
			s.score = true;
		end;
	...
	elseif found == 28 then
		p[[Вот что нашлось в заметках: "...cтаринный фамильный особняк семейства Йокиненов был построен в девятнадцатом веке, представляет собой настоящее чудо инженерной мысли. Состоит из девяти комнат на первом этаже, трёх - на втором, а также примыкающего гаража, террасы, подвала и чердака. 
		После современной реставрации особняка он оснащён по последнему слову техники. Тётя Агата получила особняк по завещанию от мужа."]];
