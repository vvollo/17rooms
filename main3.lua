...
mp.msg.StoneCode = {}
function mp:StoneCode()
    p(mp.msg.StoneCode.StoneCode)
end
mp.msg.StoneCode.StoneCode = "КФ937 -- кодовое обозначение обломка чёрного камня из пустыни Такла-Макан. И что?"
...
function mp:Authors()       
  p( 		"Идея и доработки этой версии игры: vvollo"..[[
		...
		^^И, конечно, отдельное спасибо Горафу за то, что он организовал мероприятие, а также за (шуточное?) предложение делать форки оригинальной игры.]]);
...
function mp:Advice()       
  p([[Советы по прохождению:
	...
	^- на стороны света (СЮЗВ) можно не только идти, их также возможно осматривать и даже бросать в них что-то;
	^- при выходе из комнаты обращайте внимание на текст, который выводится перед описанием новой комнаты -- его легко не заметить, но там может быть полезная информация;
	^- не забывайте осматривать стены, пол и потолок в комнатах, даже если они явно не упоминаются;
	^- команда ОТМЕНА отменяет последнее действие, она работает даже во время катсцен и диалогов;
